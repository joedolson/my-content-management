...
class mcm_posts_widget extends WP_Widget {
	...
	?>
		<input type='hidden' name='<?php echo $this->get_field_name( 'wrapper' ); ?>' value='<?php echo $wrapper; ?>' />
		<input type='hidden' name='<?php echo $this->get_field_name( 'template' ); ?>' value='<?php echo $template; ?>' />
	<?php
		}
	}
...
class mcm_meta_widget extends WP_Widget {
	function __construct() {
		parent::__construct( false,$name=__('Custom Post Data','my-content-management'), array( 'description' => __( 'Widget displaying data entered in a specific fieldset.', 'my-content-management' ), 'customize_select_refresh' => true ) );
	}
	...
	function widget($args, $instance) {
		...
		$fieldset = mcm_format_fieldset( $fieldset, $display, array( 'label'=>$left_column, 'values'=>$right_column ), $fieldset_name, $custom_template );
	...
	?>
		...
		<?php if ( $display == 'custom' ) { ?>
			...
			<textarea class="widefat" cols="70" rows="6" id="<?php echo $this->get_field_id('custom_template'); ?>" name="<?php echo $this->get_field_name('custom_template'); ?>"><?php echo esc_attr( $custom_template ); ?></textarea>
			</p>
		<?php } ?>
		</fieldset>
	<?php
...
function mcm_test_chooser_field( $id = false, $url = false, $type = false, $label = false ) {
	...
	switch ($type) {
		...
		case 'image/png':
		case 'image/gif':
			return ( $id ) ? wp_get_attachment_image( $id, 'thumbnail', true, array( 'class'=>'mcm-attachment-chooser' ) ) : "<img src='$url' alt='' />";
		case 'mp3':
		case 'ogg':
		case 'audio/mpeg':
		case 'audio/wav':
...
function mcm_format_value( $value, $type, $label ) {
	switch( $type ) {
		...
		case 'textarea' :
		case 'richtext' : $return = wpautop( stripslashes( $value ) ); break;
		case 'upload' :
		...
		case 'url' : $return = "<a href='" . esc_url( $value ) . "'><span class='mcm_url mcm_text'>Link</span> <span class='mcm_url screen-reader-text'>" . esc_html( $value ) . "</span></a>"; break;
		case 'post-relation' :
